...
export default function generateStuff(param) {
  ...
  const renderTask = (element) => {
    ...
    switch (element.priority) {
      case "HIGH":
        taskDone.innerHTML =
          '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="red" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-square"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect></svg>';
        break;
      case "MEDIUM":
        taskDone.innerHTML =
          '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="yellow" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-square"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect></svg>';
        break;
      case "LOW":
        taskDone.innerHTML =
          '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="gray" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-square"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect></svg>';
    }
    taskDone.addEventListener("click", () => {
      toDosManipulator.removeToDo(element);
  ...
  switch (param) {
    case "Inbox":
      ...
      toDosManipulator
        .getAllToDos()
        .filter((task) => task.project === "No Project")
        .forEach((element) => {
          if (!element.done) {
            renderTask(element);
          }
    ...
    case "Today":
      ...
      toDosManipulator
        .getAllToDos()
        .filter((task) => isToday(addDays(task.dueDate, 1)))
        .forEach((element) => {
          if (!element.done) {
            renderTask(element);
          }
    ...
    case "Tomorrow":
      ...
      toDosManipulator
        .getAllToDos()
        .filter((task) => isTomorrow(addDays(task.dueDate, 1)))
        .forEach((element) => {
          if (!element.done) {
            renderTask(element);
          }
    ...
    case "This Week":
      ...
      toDosManipulator
        .getAllToDos()
        .filter((task) => isThisWeek(addDays(task.dueDate, 1)))
        .forEach((element) => {
          if (!element.done) {
            renderTask(element);
          }
    ...
    default:
      ...
      toDosManipulator
        .getAllToDos()
        .filter((task) => task.project == param.name)
        .forEach((element) => {
          if (!element.done) {
            renderTask(element);
          }
...
export function openProjectDetails(element) {
  ...
  if (document.querySelector(".project-details")) {
    document
      ...
      .removeChild(document.querySelector(".project-details"));
  }
...
export function addProject() {
  ...
  if (document.querySelector(".project-form-add")) {
    document
      .querySelector(".projects-header")
      .removeChild(document.querySelector(".project-form-add"));
  }
  ...
  container.addEventListener("submit", function (event) {
    ...
    localStorage.setItem(
      "projectsArray",
      JSON.stringify(masterController.projectsArray)
    );
  });
